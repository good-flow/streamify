package com.goodflow.streamify.publisher;

import com.goodflow.streamify.publisher.resource.CloseableResource;
...
import com.goodflow.streamify.singnal.*;
...
@Slf4j
public class AsyncPublisher<T> implements Publisher<T>, Closeable {

    // -------------------------------------
    // Factory APIs
    // -------------------------------------

    public static class AsyncPublisherBuilder<T> {
        private final CloseableResource<T> resource;
        private final int batchSize = AsyncPublisher.DEFAULT_BATCH_SIZE;
        private final Executor executor = Executors.newFixedThreadPool(3);

        public AsyncPublisherBuilder(CloseableResource<T> resource) {
            this.resource = resource;
        }
        public AsyncPublisherBuilder(CloseableResource<T> resource, int batchSize, Executor executor) {
            this.resource = resource;
        }

        public AsyncPublisherBuilder<T> batchSize(int batchSize) {
            return new AsyncPublisherBuilder<>(resource, batchSize, executor);
        }

        public AsyncPublisherBuilder<T> executor(Executor executor) {
            return new AsyncPublisherBuilder<>(resource, batchSize, executor);
        }

        public AsyncPublisher<T> build() {
            return new AsyncPublisher<>(resource, batchSize, executor);
        }
    }

    public static <T> AsyncPublisherBuilder<T> file(FileLinesResource<T> file) {
        return new AsyncPublisherBuilder<>(file);
    }

    public static <T> AsyncPublisherBuilder<T> jdbc(JdbcResource<T> jdbc) {
        return new AsyncPublisherBuilder<>(jdbc);
    }
    ...
    protected final CloseableResource<T> elements; // This is our data source / generator
    protected final Executor executor; // This is our thread pool, which will make sure that our Publisher runs asynchronously to its Subscribers
    protected final int batchSize; // In general, if one uses an `Executor`, one should be nice nad not hog a thread for too long, this is the cap for that, in elements

    // -------------------------------------
    // Constructors
    // -------------------------------------

    public AsyncPublisher(final CloseableResource<T> elements, final Executor executor) {
        this(elements, DEFAULT_BATCH_SIZE, executor);
    }

    public AsyncPublisher(final CloseableResource<T> elements, final int batchSize, final Executor executor) {
    ...
    final class SubscriptionImpl implements Subscription, Runnable {
            ...
            try {
                ...
                do {
                    ...
                    if (!hasNext) { // If we are at End-of-Stream
                        ...
                        onComplete(); // Then we signal `onComplete` as per rule 1.2 and 1.5
                    }
                }
                while (cancelled .get() == false  // This makes sure that rule 1.8 is upheld, i.e. we need to stop signalling "eventually"
                ...
                if (cancelled.get() == false && demand > 0) {
        ...
        private void releaseResource() {
            if (resourceClosed.getAndSet(true) == false) {
                try {
                    log.info("Releasing the resource for subscriber: {}", subscriber);
                    elements.close();
                } catch (IOException e) {
                    log.warn("Failed to close resource for subscriber: {}", subscriber, e);
                }
            }
        }
        ...
        private void terminateDueTo(final Throwable t) {
            cancelled.set(true); // When we signal onError, the subscription must be considered as cancelled, as per rule 1.6
            releaseResource();
            try {
        ...
        private void signal(final Signal signal) {
            if (inboundSignals.offer(signal)) {
                // No need to null-check here as ConcurrentLinkedQueue does this for us
